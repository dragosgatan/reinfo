...
(async () => {
    ...
    // reinfo patch: piston's stock body-parser defaults to a 100kb limit with no
    // config knob to raise it, so any test case whose stdin pushes the request past
    // that silently gets a generic 400 (piston's error handler below can't tell a
    // payload-too-large error from a malformed request) and the submission is stuck
    // at "pending" forever. raised to 10mb to cover large test inputs.
